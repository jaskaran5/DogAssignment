...
ResponseWrapper<T> getFailedResponseWrapper<T>(dynamic e, {dynamic response}) =>
    ResponseWrapper(response: 0, errorMessage: e.toString(), data: response);
...
String exceptionHandler({required Object e, required String functionName}) {
  if (e is AuthenticationException) {
    ...
    blocLog(msg: e.message, bloc: "SocketException in ==>$functionName");
    return e.message;
    ...
    blocLog(msg: e.toString(), bloc: functionName);
    return e.toString();
  }
}
...
String getHandleFirebaseError({
  ...
  if (error == "invalid-phone-number") {
    ...
    functionLog(msg: "invalid code", fun: functionName);
    return "invalid code";
    ...
    functionLog(msg: "Wrong email/password combination.", fun: functionName);
    return "Wrong email/password combination.";
  } else if (error == "ERROR_USER_NOT_FOUND" || error == "user-not-found") {
    functionLog(msg: "No user found with this email.", fun: functionName);
    return "No user found with this email.";
  } else if (error == "ERROR_USER_DISABLED" || error == "user-disabled") {
    functionLog(msg: "User disabled.", fun: functionName);
    ...
    functionLog(msg: "Email address is invalid.", fun: functionName);
    return "Email address is invalid.";
  } else {
    functionLog(msg: "Login failed. Please try again.", fun: functionName);
