...
class HttpService {
  ...
  Future<ResponseWrapper<T?>> request<T>({
    ...
    try {
      ...
      if (err is DioException) {
        if (err.type == DioExceptionType.receiveTimeout ||
          ...
          return ResponseWrapper(
            response: 0,
            errorMessage: "Unable to reach the servers",
          );
      ...
      if (res != null && res?.data.runtimeType != String) {
        return ResponseWrapper.fromJson(res?.data, (data) => null);
      }
  ...
  Future<Response> _checkRequest({
    ...
    if (requestType == RequestType.get) {
      return _dio.get(fullUrl, options: headerToken, queryParameters: body);
    } else if (requestType == RequestType.post) {
      return _dio.post(fullUrl, data: body, options: Injector.getHeaderToken());
      ...
      if (imagePath.isNotEmpty) {
        ...
        map[paramName] = await MultipartFile.fromFile(
          file.path,
      ...
      return _dio.delete(fullUrl, data: body, options: headerToken);
    } else if (requestType == RequestType.put) {
      return _dio.put(fullUrl, data: body, options: headerToken);
    } else {
      return _dio.patch(fullUrl, data: body, options: headerToken);
    }
  }
